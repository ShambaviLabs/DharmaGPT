...
GET    /api/v1/feedback/pending          — upvoted responses awaiting review (API-key gated)
PATCH  /api/v1/feedback/{query_id}       — approve or reject a response (API-key gated)
GET    /api/v1/feedback/gold             — list approved gold responses (API-key gated)
"""
...
async def list_gold(x_api_key: str | None = Header(None)) -> dict:
    _require_key(x_api_key)
